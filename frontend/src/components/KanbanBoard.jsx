    ...
    <>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-slate-400">{projectKey}-{task.id}</span>
          {commentCount > 0 && (
        ...
        <div className="flex -space-x-2">
          {(task.assignees || []).slice(0, 3).map((a) => (
            <div key={a.id} title={a.name}
              className={`flex h-6 w-6 items-center justify-center rounded-full border-2 border-white text-[10px] font-semibold text-white ${avatarColor(a.name)}`}>
              {initials(a.name)}
            </div>
          ))}
          {(task.assignees || []).length > 3 && (
            <div className="flex h-6 w-6 items-center justify-center rounded-full border-2 border-white bg-slate-400 text-[10px] font-semibold text-white">
              +{task.assignees.length - 3}
            </div>
          )}
        </div>
...
function SortableTaskCard({ task, projectKey, isDragDisabled, onOpen }) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners}
      ...
      className={`mb-2.5 cursor-pointer rounded-lg border bg-white p-3 shadow-sm transition-shadow hover:border-indigo-200 hover:shadow-md ${
        isDragging ? "border-indigo-300 ring-2 ring-indigo-200" : "border-slate-200"}`}>
  ...
  return (
    <div ref={setNodeRef}
      className={`w-[272px] shrink-0 rounded-xl border p-2.5 transition-colors ${
        isOver ? "border-indigo-300 bg-indigo-50/70" : "border-slate-200/70 bg-slate-50"}`}>
      <div className="mb-3 flex items-center justify-between px-1">
        <div className="flex items-center gap-2">
          <span className={`h-2 w-2 rounded-full ${meta.dot}`} />
          <span className="text-xs font-bold uppercase tracking-wide text-slate-500">{meta.label}</span>
        </div>
        <span className="rounded-full bg-slate-200/80 px-2 py-0.5 text-xs font-medium text-slate-600">{items.length}</span>
      </div>
...
export default function KanbanBoard({ tasks, onStatusChange, canEdit = true, projectKey = "TASK", onOpenTask }) {
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={onDragStart} onDragEnd={onDragEnd}>
      ...
      <DragOverlay>
        {activeTask && (
          <div className="w-[272px] rotate-2 rounded-lg border border-indigo-300 bg-white p-3 shadow-xl ring-2 ring-indigo-100">
