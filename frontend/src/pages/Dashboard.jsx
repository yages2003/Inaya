...
function ProjectCard({ project, tasksForProject, onOpen }) {
  const meta = PROJECT_STATUS[project.status] || { label: project.status, bg: "bg-slate-100", text: "text-slate-600", solid: "bg-slate-400" };
    ...
    >
      <div className="mb-2 flex items-start justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className="truncate text-lg font-semibold text-slate-800">{project.name}</span>
        </div>
        <span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-medium ${meta.bg} ${meta.text}`}>{meta.label}</span>
      </div>
      <p className="min-h-10 text-sm text-slate-500 line-clamp-2">{project.description || "No description"}</p>
      ...
      <p className="mt-3 text-xs text-slate-400">{project.start_date} → {project.end_date}</p>
    </div>
  );
}
    ...
    <>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">Overview of all projects and work.</p>
        </div>
        {can("project_create") && (
      ...
      {loading ? (
        ...
        <>
          <div className="mb-8 grid grid-cols-2 gap-4 md:grid-cols-4">
            <StatCard icon={<IconFolders size={22} />} label="Projects" value={stats.total} color={{ bg: "bg-indigo-50", text: "text-indigo-600" }} />
            <StatCard icon={<IconChecklist size={22} />} label="Open tasks" value={stats.openTasks} color={{ bg: "bg-blue-50", text: "text-blue-600" }} />
            <StatCard icon={<IconAlertTriangle size={22} />} label="Needs attention" value={stats.atRisk} color={{ bg: "bg-amber-50", text: "text-amber-600" }} />
            <StatCard icon={<IconCircleCheck size={22} />} label="Completed" value={stats.completed} color={{ bg: "bg-emerald-50", text: "text-emerald-600" }} />
          </div>

          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <h4 className="text-lg font-semibold text-slate-800">Projects</h4>
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-[220px] rounded-lg border border-slate-200 py-1.5 pl-9 pr-3 text-sm outline-none focus:border-indigo-400"
                />
              </div>
              <div className="flex rounded-lg bg-slate-100 p-1 text-xs">
                {filters.map((f) => (
                  <button key={f.value} onClick={() => setStatusFilter(f.value)}
                    className={`rounded-md px-2.5 py-1 font-medium transition ${statusFilter === f.value ? "bg-white text-slate-800 shadow-sm" : "text-slate-500"}`}>
                    {f.label}
                  </button>
                ))}
              </div>
          ...
          {filtered.length === 0 ? (
            <div className="flex h-56 flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-slate-200 bg-white/60">
              ...
              {projects.length === 0 && can("project_create") && (
                <button onClick={() => setOpened(true)}
                  className="flex items-center gap-1.5 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-1.5 text-sm font-medium text-indigo-600 transition-colors hover:bg-indigo-100">
