    ...
    <>
      <div className="mb-6 flex items-center justify-between no-print">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">Executive and operational narratives for a project.</p>
        </div>
        {summary && (
      ...
      <div className="mb-6 no-print">
        ...
        <select value={projectId} onChange={(e) => setProjectId(e.target.value)}
          className="w-full max-w-sm rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-700 transition-colors focus-ring dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200">
      ...
      {!loading && !projectId && (
        <div className="flex flex-col items-center gap-3 py-16 text-center no-print">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">Select a project above to view its report.</p>
        </div>
      )}
      ...
      {!loading && projectId && !summary && (
        <div className="flex flex-col items-center gap-3 py-16 text-center no-print">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">No AI insights available for this project yet. Generate insights from its AI Insights tab first.</p>
        </div>
      )}

      {!loading && summary && (
        <div>
          <div className="mb-6 border-b border-slate-200 pb-4 print:mb-4 dark:border-slate-700">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <div className="card p-5">
              <div className="mb-3 flex items-center gap-2">
                <IconUserCircle size={18} className="text-indigo-600 dark:text-indigo-400" />
                <h4 className="font-semibold text-slate-800 dark:text-slate-100">Executive Narrative</h4>
              </div>
              <p className="text-sm leading-relaxed text-slate-700 dark:text-slate-300">{summary.executive_narrative || "No executive narrative available."}</p>
            </div>
            <div className="card p-5">
              <div className="mb-3 flex items-center gap-2">
                <IconTargetArrow size={18} className="text-indigo-600 dark:text-indigo-400" />
                <h4 className="font-semibold text-slate-800 dark:text-slate-100">Operational Narrative</h4>
              </div>
              <p className="text-sm leading-relaxed text-slate-700 dark:text-slate-300">{summary.operational_narrative || "No operational narrative available."}</p>
            </div>
          </div>
        </div>
      )}
