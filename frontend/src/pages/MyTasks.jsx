  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition-shadow hover:shadow-md dark:border-slate-700 dark:bg-slate-800">
      ...
      <div className="flex flex-wrap gap-1.5">
        <span className="rounded bg-slate-100 px-1.5 py-0.5 text-xs text-slate-600 dark:bg-slate-700 dark:text-slate-300">{TASK_CATEGORY[t.category]}</span>
...
export default function MyTasks() {
  ...
  return (
    <>
      <h2 className="mb-1 text-2xl font-bold text-slate-800 dark:text-slate-100">My tasks</h2>
      <p className="mb-6 text-sm text-slate-500 dark:text-slate-400">Everything assigned to {user?.name}.</p>
      {tasks.length === 0 ? (
        <div className="flex h-56 flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-slate-200 bg-white/60 dark:border-slate-700 dark:bg-slate-800/60">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">You have no assigned tasks right now.</p>
        </div>
      ) : (
        <div className="space-y-8">
          <div>
            <p className="mb-3 font-semibold text-slate-800 dark:text-slate-100">Active ({active.length})</p>
            {active.length === 0 ? (
              <p className="text-sm text-slate-500 dark:text-slate-400">Nothing active. Nice work.</p>
            ) : (
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {done.length > 0 && (
            <div>
              <p className="mb-3 font-semibold text-slate-800 dark:text-slate-100">Completed ({done.length})</p>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
