...
export default function ProjectDetail() {
  ...
  if (loading) return (
    <div className="animate-fade-in">
      ...
      <div className="mb-4 flex gap-1 border-b border-slate-200 pb-2">
  ...
  if (!project) return (
    <div className="flex h-[300px] flex-col items-center justify-center gap-3 text-center">
      ...
      <Link to="/" className="text-sm font-medium text-indigo-600 hover:underline">Back to projects</Link>
    </div>
  );

  const meta = PROJECT_STATUS[project.status] || { label: project.status, bg: "bg-slate-100", text: "text-slate-600" };
  ...
  return (
    <>
      <nav className="mb-2 flex items-center gap-1.5 text-sm text-slate-500">
        <Link to="/" className="font-medium text-indigo-600 transition-colors hover:text-indigo-700 hover:underline">Projects</Link>
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <Link to="/" className="rounded-lg p-2 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600">
      ...
      {project.description && <p className="mb-5 text-slate-500">{project.description}</p>}
          ...
          {TABS.map((t) => (
            <button key={t.value} onClick={() => setTab(t.value)}
              className={`relative flex items-center gap-1.5 px-3 py-2.5 text-sm font-medium transition-colors ${
                tab === t.value ? "text-indigo-600" : "text-slate-500 hover:text-slate-700"}`}>
              <t.icon size={16} /> {t.label}
              {tab === t.value && <span className="absolute inset-x-0 -bottom-px h-0.5 rounded-full bg-indigo-600" />}
            </button>
          ))}
        </div>
        {(tab === "board" || tab === "timeline") && (
          <div className="flex items-center gap-2 pb-2">
            <IconFlag3Filled size={13} className="text-slate-400" />
            <div className="flex rounded-lg bg-slate-100 p-1 text-xs">
              {PRIORITY_FILTERS.map((f) => (
                <button key={f.value} onClick={() => setPriorityFilter(f.value)}
                  className={`rounded-md px-2.5 py-1 font-medium transition-colors ${priorityFilter === f.value ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
                  {f.label}
                </button>
              ))}
            </div>
        ...
        <div className="animate-fade-in">
          ...
          <div className="card grid grid-cols-1 gap-4 p-5 sm:grid-cols-2">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-700">{project.start_date} → {project.end_date}</p>
            </div>
            <div>
      ...
      {tab === "board" && (
        <div className="animate-fade-in">
          ...
          <p className="mt-2 text-xs text-slate-400">Tip: drag a card between columns to change its status.</p>
        </div>
      )}
        ...
        <div className="card animate-fade-in p-5">
          {!activityLoaded ? (
            ...
            <div className="flex flex-col items-center gap-2 py-8 text-center">
              ...
              <p className="text-sm text-slate-500">No activity recorded for this project yet.</p>
            </div>
          ) : (
        ...
        <div className="space-y-4">
          <div>
            ...
            <input required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })}
              className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-700 transition-colors focus-ring" />
          </div>
          <div>
          ...
          <div>
            ...
            <select multiple value={form.assignee_ids}
              ...
              className="h-28 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-700 transition-colors focus-ring">
        ...
        {activeTask && (
          <div className="space-y-4">
            <div className="flex items-start justify-between gap-3">
              <h3 className="text-lg font-semibold text-slate-800">{activeTask.title}</h3>
              {can("task_delete") && (
                <button onClick={removeTask} disabled={deleting}
                  className="flex shrink-0 items-center gap-1.5 rounded-lg border border-red-200 bg-red-50 px-2.5 py-1.5 text-xs font-semibold text-red-600 transition-colors hover:bg-red-100 disabled:opacity-60">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge className="bg-slate-100 text-slate-600">{TASK_CATEGORY[activeTask.category]}</Badge>
              {activeTask.due_date && (() => {
                ...
                <div className="flex -space-x-2">
                  {activeTask.assignees.map((a) => (
                    <div key={a.id} title={a.name}
                      className={`flex h-7 w-7 items-center justify-center rounded-full border-2 border-white text-xs font-semibold text-white ${avatarColor(a.name)}`}>
                      {initials(a.name)}
                    </div>
                  ))}
                </div>
            ...
            <div className="border-t border-slate-100 pt-3">
              <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">Comments</p>
              ...
              {can("comment_create") && (
                <div className="flex items-end gap-2">
                  <textarea placeholder="Write a comment…" rows={1} value={commentText}
                    ...
                    className="flex-1 rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-700 transition-colors focus-ring" />
